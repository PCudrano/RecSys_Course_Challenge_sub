...
class AlbumBoostRecommender(Recommender):

    RECOMMENDER_NAME = "AlbumBoostRecommender"

    def __init__(self, URM_train, test_interactions_df, tracks_df, eurm):
        super(AlbumBoostRecommender, self).__init__()
        self.URM_train = check_matrix(URM_train, 'csr')
        self.test_interactions_df = test_interactions_df
        self.tracks_df = tracks_df
        self.eurm = eurm

    def fit(self, targets, k, gamma, norm=norm_l1_row):
        self.ab = AlbumBoost(self.test_interactions_df, self.tracks_df, self.URM_train, self.eurm, norm=norm)
        self.mine_est_ratings = self.ab.boost_eurm(targets, k, gamma)
