...
from Boosters.TailBoostRecommender import TailBoostRecommender
from Boosters.AlbumBoostRecommender import AlbumBoostRecommender
...
def runParameterSearch_Collaborative(recommender_class, URM_train, ICM_all=None,
    ...
    try:
       ...
       ##########################################################################################################
        ...
        if recommender_class is TailBoostRecommender:
            def get_sequences_df(URM, train_sequential_df, user_arr):
                ...
                for user_id in user_arr:
                    ...
                    if user_nnz > 0:
                        ...
                        original_order_items = np.array([i for i in original_order_items if i in URM.indices[
                                                                                                 URM_start_pos:URM_end_pos]])  # filter items in urm
            ...
            hyperparamethers_range_dictionary = {}
            hyperparamethers_range_dictionary["last_tracks"] = Integer(1,20)
            hyperparamethers_range_dictionary["k"] = Integer(5, 1000)
            hyperparamethers_range_dictionary["gamma"] = Real(low=1e-4, high=1, prior='log-uniform')
            hyperparamethers_range_dictionary["norm"] = Categorical([norm_l1_row, norm_l2_row, norm_max_row])

            recommenderDictionary = {
                DictionaryKeys.CONSTRUCTOR_POSITIONAL_ARGS: [URM_train, train_sequences, total_est_ratings, similarity],
                DictionaryKeys.CONSTRUCTOR_KEYWORD_ARGS: {},
                DictionaryKeys.FIT_POSITIONAL_ARGS: [targetsListOrdered],
                DictionaryKeys.FIT_KEYWORD_ARGS: {},
                DictionaryKeys.FIT_RANGE_KEYWORD_ARGS: hyperparamethers_range_dictionary}

        ##########################################################################################################

        if recommender_class is AlbumBoostRecommender:
            def get_sequences_df(URM, train_sequential_df, user_arr):
                sequences_arr = []
                for user_id in user_arr:
                    URM_start_pos = URM.indptr[user_id]
            ...
            hyperparamethers_range_dictionary = {}
            hyperparamethers_range_dictionary["k"] = Integer(1, 1000)
            hyperparamethers_range_dictionary["gamma"] = Real(low=1e-2, high=100, prior='log-uniform')
            hyperparamethers_range_dictionary["norm"] = Categorical([norm_l1_row, norm_l2_row, norm_max_row])

            recommenderDictionary = {
                DictionaryKeys.CONSTRUCTOR_POSITIONAL_ARGS: [URM_train, train_sequences, tracks_df, total_est_ratings],
                DictionaryKeys.CONSTRUCTOR_KEYWORD_ARGS: {},
                DictionaryKeys.FIT_POSITIONAL_ARGS: [targetsListOrdered],
                DictionaryKeys.FIT_KEYWORD_ARGS: {},
                DictionaryKeys.FIT_RANGE_KEYWORD_ARGS: hyperparamethers_range_dictionary}
...
if __name__ == '__main__':
    ...
    with open('../../../dump/dump_URM_test_rowholdout0802th2', 'rb') as dump_file:
        URM_valid_test_pred = pickle.load(dump_file)
    ...
    collaborative_algorithm_list = [
        ...
        # LightFMRecommender,
        # FPMCRecommender,
        # TailBoostRecommender,
        AlbumBoostRecommender
    ]
    ...
    evaluator_validation = FastEvaluator(URM_validation, cutoff_list=[10], minRatingsPerUser=1, exclude_seen=True, ignore_users=users_excluded_targets)
    evaluator_test = FastEvaluator(URM_test, cutoff_list=[10], minRatingsPerUser=1, exclude_seen=True, ignore_users=userList_unique[:-2]) # to be fast
