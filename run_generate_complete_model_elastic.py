...
if __name__ == '__main__':
    ...
    # recommender_class = SLIMLinearRegressionRecommender
    ...
    try:
        ...
        if recommender_class is SLIM_BPR_Cython:
        ...
        elif recommender_class is ImplicitALSRecommender:
            print("Init recsys")
            recommender = recommender_class(URM_train)
            print("Fitting recsys")
            recommender.fit(alpha=12, factors=600, regularization=0.05388, iterations=80)
            print("Hopefully done")
            recommender.saveEstRatings("result_experiments/hyb_est_ratings_6/", file_name="ALS_complete_est_rat_2", user_id_array=userList_unique, k=160)
            print("Hopefully saved")

        elif recommender_class is SLIMLinearRegressionRecommender:
            recommender = recommender_class(URM_train)
            # generate elastic_net_2/SLIMLinearRegressionRecommender_best_model
            # {'topK': 151, 'alpha': 4.9301163286885506e-05}
            recommender.fit(topK=151,
                            alpha=4.9301163286885506e-05,
                            reg_type="l1",  # l2 too slow because not sparse
                            solver='auto',
                            positive_only=False,
                            max_iter=None,
                            selection="random",
            ...
            recommender.saveModel("result_experiments/elastic_net_2/", file_name="SLIMLinearRegressionRecommender_complete")

        elif recommender_class is SLIMElasticNetRecommender:
            recommender = recommender_class(URM_train)
            # generate elastic_net_3/SLIMElasticNetRecommender_best_model
            # {'topK': 940, 'l1_ratio': 0.3115917591020172, 'alpha': 4.548317507617178e-05}
            recommender.fit(topK=940,
                            l1_ratio=0.3115917591020172,
                            alpha=4.548317507617178e-05,
                            positive_only=True,
                            max_iter=1000,
                            selection="random",
                            tol=1e-3,
                            )
            recommender.saveModel("result_experiments/elastic_net_3/", file_name="SLIMElasticNetRecommender_complete")
