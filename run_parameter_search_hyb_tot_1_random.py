...
#                                      output_root_path ="result_experiments/", parallelizeKNN = False, n_cases = 30):
    ...
    try:
        ...
        parameterSearch = RandomSearch(recommender_class, evaluator_validation=evaluator_validation, evaluator_test=evaluator_test)
       ...
       ##########################################################################################################
        ...
        if recommender_class is HybridLinCombEstRatings:
            ...
            recsys[-3].loadModel(slims_dir, "SLIM_BPR_Recommender_best_model_300")
            print("Load complete of slim bpr")
            el_t = time.time() - t
            print("Done. Elapsed time: {:02d}:{:06.3f}".format(int(el_t / 60), el_t - 60 * int(el_t / 60)))

            print("Starting fitting als")
            recsys[-2].fit(alpha=15, factors=495, regularization=0.04388, iterations=20)
            ...
            recsys_est_ratings.append(recsys[-2].estimate_ratings(userList_unique, 160))
            ...
            for i in range(0, N_hyb):
                text = "alphas" + str(i)
                hyperparamethers_range_dictionary[text] = (0.0, 40.0)
        ...
        best_parameters = parameterSearch.search(recommenderDictionary,
                                                 n_cases = n_cases,
                                                 output_root_path = output_root_path_rec_name,
                                                 metric = metric_to_optimize, parallelize=False)
...
def make_hyb_item_sim(URM_train, norm=None):
    print("Starting importing everything")
    ...
    for i in range(0, N_hyb):
        text = "alphas" + str(i)
        alphases[text] = alphas[i]

    if norm is None:
        norm = N_hyb

    hyb_recsys = HybridLinCombItemSimilarities(URM_train, recsys)
    hyb_recsys.fit(aa=7, **alphases)
    est_ratings = hyb_recsys.estimate_ratings(userList_unique, 160)
    est_ratings = est_ratings / norm
    return est_ratings
...
def read_data_split_and_search(parallel=False):
    ...
    output_root_path = "result_experiments/tuning_{date:%Y%m%d%H%M%S}_tot/".format(date=datetime.datetime.now())
