...
if __name__ == '__main__':
    ...
    try:
        ...
        gamma = 0.01
        norm = norm_max_row
        recommender_boost = AlbumBoostRecommender(URM_train, train_sequences, tracks_df, recommender.mine_est_ratings)
        recommender_boost.fit(targetsListOrdered, k, gamma, norm)

        recommendations_seq = np.array(recommender_boost.recommend(targetsListOrdered, cutoff=10, remove_seen_flag=True)).tolist()
        ...
        # Custom name
        csv_filename = "hybrid_est_ratings_54_boost"
        # Default name
