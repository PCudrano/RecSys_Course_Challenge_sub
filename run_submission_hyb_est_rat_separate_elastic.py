...
if __name__ == '__main__':
    ...
    try:
        ...
        recsys[-6].loadModel(slims_dir, "SLIM_BPR_complete")
        print("Load complete of slim bpr")
        el_t = time.time() - t
        print("Done. Elapsed time: {:02d}:{:06.3f}".format(int(el_t / 60), el_t - 60 * int(el_t / 60)))
        ...
        recsys[-5].loadModel(slims_dir, "SLIMLinearRegressionRecommender_complete")
        ...
        recsys[-4].loadModel(slims_dir, "SLIMElasticNetRecommender_complete")
        print("Load complete of slim elasticnet")
        el_t = time.time() - t
        print("Done. Elapsed time: {:02d}:{:06.3f}".format(int(el_t / 60), el_t - 60 * int(el_t / 60)))
        ...
        slims_dir = "result_experiments/hyb_est_ratings_6/"
        recsys_est_ratings.append(recsys[-3].loadEstRatings(slims_dir, "ALS_complete_est_rat")[0])
        recsys_est_ratings.append(recsys[-2].loadEstRatings(slims_dir, "ALS_complete_est_rat_2")[0])
        el_t = time.time() - t2
        ...
        # # best tot sub 56
        # a = {'alphas0': 11.05582264699394, 'alphas1': 34.157371147183014, 'alphas2': 99.66856408476876,
        #  'alphas3': 0.18755415432473205, 'alphas4': 0.8641137081047058, 'alphas5': 0.21781006059262092,
