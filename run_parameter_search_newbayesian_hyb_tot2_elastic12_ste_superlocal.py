    ...
    try:
       ...
       ##########################################################################################################
        ...
        if recommender_class is HybridLinCombEstRatings:
            ...
            # {'alphas0': 13.718596681389458, 'alphas1': 28.891119601707942, 'alphas2': 70.78984416850315, 'alphas3': 0.0,
            ...
            #  'alphas12': 118.95603165055005, 'alphas13': 142.0455597041231}

            hyperparamethers_range_dictionary["alphas0"] = Real(low=11.5, high=13.0)
            hyperparamethers_range_dictionary["alphas1"] = Real(low=29.0, high=31.0)
            hyperparamethers_range_dictionary["alphas2"] = Real(low=78.0, high=81.0)
            hyperparamethers_range_dictionary["alphas3"] = Real(low=0.0, high=2.0)
            hyperparamethers_range_dictionary["alphas4"] = Real(low=0.0, high=2.0)
            hyperparamethers_range_dictionary["alphas5"] = Real(low=0.0, high=2.0)
            hyperparamethers_range_dictionary["alphas6"] = Real(low=109.0, high=112.0)
            hyperparamethers_range_dictionary["alphas7"] = Real(low=0.0, high=1.0)
            hyperparamethers_range_dictionary["alphas8"] = Real(low=43.0, high=46.0)
            hyperparamethers_range_dictionary["alphas9"] = Real(low=43.0, high=46.0)
            hyperparamethers_range_dictionary["alphas10"] = Real(low=1.0, high=4.0)
            hyperparamethers_range_dictionary["alphas11"] = Real(low=110.0, high=150.0)
            hyperparamethers_range_dictionary["alphas12"] = Real(low=110.0, high=150.0)
            hyperparamethers_range_dictionary["alphas13"] = Real(low=140.0, high=150.0)

            # # enet
            # hyperparamethers_range_dictionary["alphas" + str(N_hyb - 5)] = Real(low=0.0, high=100.0)
            # hyperparamethers_range_dictionary["alphas" + str(N_hyb - 4)] = Real(low=50.0, high=200.0)
            #
            #
            # # # als
            # hyperparamethers_range_dictionary["alphas"+str(N_hyb-3)] = Real(low=90.0, high=200.0)
            ...
            recommenderDictionary = {DictionaryKeys.CONSTRUCTOR_POSITIONAL_ARGS: [URM_train, recsys_est_ratings],
                                     DictionaryKeys.CONSTRUCTOR_KEYWORD_ARGS: {},
                                     DictionaryKeys.FIT_POSITIONAL_ARGS: dict(),
                                     DictionaryKeys.FIT_KEYWORD_ARGS: {"alphas"+str(N_hyb-1) : 0.3},
                                     DictionaryKeys.FIT_RANGE_KEYWORD_ARGS: hyperparamethers_range_dictionary}
...
if __name__ == '__main__':
    ...
    output_root_path = "result_experiments/tuning_skopt_{date:%Y%m%d%H%M%S}_tot2_elastic12_ste_local/".format(date=datetime.datetime.now())
