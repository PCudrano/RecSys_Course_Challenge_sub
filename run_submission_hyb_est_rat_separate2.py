...
if __name__ == '__main__':
    ...
    try:
        ...
        N_mix_markov = 1
        N_als = 1
        N_hyb = N_cbf + N_cf + N_p3a + N_ucf + N_ucbf + N_rp3b + N_slim + N_mix_markov + N_als
        recsys = []
        for i in range(N_cbf):
            recsys.append(ItemCBFKNNRecommender(URM_train, ICM_all))
        for i in range(N_cf):
        ...
        # load slim bpr
        slims_dir = "src/libs/RecSys_Course_2018/result_experiments/hyb_est_ratings_6/"
        recsys[-3].loadModel(slims_dir, "SLIM_BPR_complete")
        print("Load complete of slim bpr")
        el_t = time.time() - t
        print("Done. Elapsed time: {:02d}:{:06.3f}".format(int(el_t / 60), el_t - 60 * int(el_t / 60)))
        ...
        # best seq
        a = {'alphas0': 51.516810944663405, 'alphas1': 12.380808592905483, 'alphas2': 60.0, 'alphas3': 0.0,
             'alphas4': 60.0, 'alphas5': 13.284037351874618, 'alphas6': 60.0, 'alphas7': 8.245649227252295,
             'alphas8': 21.847689807495588, 'alphas9': 60.0, 'alphas10': 0.0, 'alphas11': 0.0,
             'alphas12': 13.050930442996723, 'alphas13': 60.0, 'alphas14': 60.0}
